...
// Fetch Budget Data
$budgetQuery = "SELECT BudgetID, Month, budgetyear FROM budget WHERE userID = ?";  // Assuming the budgets are user-specific
...
if (isset($_POST['add_subscription'])) {
    $category = $_POST['subscription_category'];
    ...
    $stmt = $db->prepare($budgetQuery);
    $stmt->bind_param("i", $budgetID); // "i" for integer parameter
    $stmt->execute();
    $budgetResult = $stmt->get_result();
    
    if ($budgetResult->num_rows > 0) {
        // Get the budget month and year from the result
        $budget = $budgetResult->fetch_assoc();
        $budgetMonth = $budget['Month'];  // e.g., "January"
        $budgetYear = $budget['budgetyear']; // e.g., 2025

        // Insert the subscription with the correct budget month and year
        $query = "INSERT INTO Subscription (userID, Category, Amount, PayingDate, RenewalDate, BudgetID) 
                  VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = $db->prepare($query);
        $stmt->bind_param("ssdsds", $userID, $category, $amount, $payingDate, $renewalDate, $budgetID);
        if ($stmt->execute()) {
        ...
        <div class="form-container">
            <form method="POST">
                <div class="input-group">
                    ...
                    <select id="subscription_category" name="subscription_category" class="form-control" required>
                ...
                <div class="input-group">
                    ...
                    <input type="date" class="form-control" id="subscription_paying_date" name="subscription_paying_date" required>
                </div>
                <div class="input-group">
                    <label for="subscription_renewal_date">Renewal Date</label>
                    <input type="date" class="form-control" id="subscription_renewal_date" name="subscription_renewal_date" required>
                </div>
